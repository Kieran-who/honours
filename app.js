...
//default as shown on the openai playground page
const chatOPENAIZERPParams = {
    temperature: 0,
    top_p: 1,
};

const stuffDoer = async (arr, cnt, sampleTotal) =>{
...
googBisonChat(arr, cnt, 100, sampleTotal, googleDefaultParams);

// ZERO MODEL PARAMETERS
// GET RESULTS FROM GPT3.5-TURBO
gptThreeFiveTurbo(arr, cnt, sampleTotal, chatOPENAIZERPParams);
// GET RESULTS FROM GPT4
gptFour(arr, cnt, sampleTotal, chatOPENAIZERPParams);
// GET RESULTS FROM GOOGLE TEXT BISON
googBisonQuick(arr, cnt, 100, sampleTotal, googleZeroParams)
// GET RESULTS FROM GOOGLE CHAT BISON
googBisonChat(arr, cnt, 100, sampleTotal, googleZeroParams);
}

gptThreeFiveTurbo(rQuestions, 0, 100, chatGPT35DefaultParams);